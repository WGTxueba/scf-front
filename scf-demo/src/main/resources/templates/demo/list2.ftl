    ...
    $(function () {
        var jqGrid = $("#jqGrid").jqGrid({
            ...
            colModel: [
                ...
                {label: '', name: 'id'},
                {label: '测试名称', name: 'name'},
                {label: '性别', name: 'sex'},
                {label: '测试登录名称', name: 'loginName'},
            ],
        })
        ...
        $(document).on('click','button[id="update"],a.update',function () {
            ...
            location.href = basePath + "/demo/demo2/" + rowId;
        });
        ...
        $(document).on('click', 'button[id="delete"],a.delete', function () {
            ...
            if (rowId.length === 1) {
                $.btDelete(basePath + "/api/demo/demo2/" + rowId, {}, function () {
                    //刷新表格
                    jqGrid.refresh();
                    //重置多选项
                ...
                $.btPost(basePath + "/api/demo/demo2/batchDelete", rowId, function () {
                    jqGrid.refresh();
                    jqGrid.resetSelection();
                    alert("删除成功！");
